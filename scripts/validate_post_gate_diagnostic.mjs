...
const outDir =
  "/home/ubuntu/primegrowth-website/validation/post-gate-diagnostic";
...
const cases = [
  {
    lang: "en",
    url: "http://localhost:3000/resources/construction-systems/thank-you?email=test@example.com&name=David",
  ...
  {
    lang: "fr",
    url: "http://localhost:3000/fr/ressources/systemes-construction/merci?email=test@example.com&name=David",
...
for (const testCase of cases) {
  ...
  if (!startText.includes(testCase.expectedStart)) {
    throw new Error(`${testCase.lang}: missing diagnostic start copy`);
  }
  const normalizedStartText = startText.toLowerCase();
  if (
    !normalizedStartText.includes(
      testCase.lang === "en" ? "your unlocked tools" : "tes outils débloqués"
    )
  ) {
    throw new Error(`${testCase.lang}: unlocked resource section missing`);
  }
  ...
  if (!resultText.includes(testCase.expectedResult)) {
    throw new Error(`${testCase.lang}: expected result missing`);
  }
  if (!resultText.includes(testCase.expectedTool)) {
    throw new Error(`${testCase.lang}: expected recommendation CTA missing`);
  }

  results.push({
    lang: testCase.lang,
    startVisible: true,
    resourceAccessStillVisible: true,
    resultVisible: true,
